...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const tool = getToolBySlug("markdown", locale);
  if (!tool) {
...
export default async function MarkdownWorkbenchPage({ params }: Props) {
  const { locale } = await params;
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <Breadcrumb
        ...
        items={[{ name: tool.title, url: `${SITE_URL}/${locale}/tools/markdown` }]}
      />
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <FavoritesButton toolSlug="markdown" iconOnly={false} className="mt-1" />
        </div>
        <p className="text-zinc-600 max-w-2xl">{tool.description}</p>
      </div>

      {/* Tool Component (client-side renderer) */}
      <ToolRenderer slug="markdown" />
      ...
      {/* Similar Tools */}
      <SimilarTools slug="markdown" locale={locale} />
